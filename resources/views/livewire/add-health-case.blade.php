<div>
    @section('title', 'Patient Health Cases')
    ...
    @if ($isValid)
        ...
        <div>
            <span class="text-2xl font-semibold">Cases</span>
            <div>
                <div class="mt-6 border-t border-gray-100">
                  <dl class="divide-y divide-gray-100">
                    @forelse ($patient->healthCases as $record)
                    <div class="bg-gray-50 px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-3">
                        {{-- <dt class="text-sm/6 font-medium text-gray-900">Record: </dt> --}}
                        <dd class="mt-1 text-sm/6 text-gray-700 sm:col-span-2 sm:mt-0 uppercase">{{$record->health_case.' - ('. Carbon\Carbon::parse($record->created_at)->format('F d, Y').')'}}</dd>
                      </div>   
                    @empty
        ...
        <div class="flex mt-4 justify-end space-x-4">
            <a href="{{ route('admin.cases') }}" class="px-4 py-3 bg-gray-50 text-gray-500 font-semibold rounded-lg border border-gray-400">Cancel</a>
            <button wire:click="submitHealthCase" wire:confirm="Are you sure you want to submit this information?" class="px-4 py-3 bg-blue-500 text-white font-semibold rounded-lg">Submit</button>
        </div>
    @endif
</div>
