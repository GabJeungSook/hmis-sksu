<div  id="print-area">
    ...
    <div class="border-b pb-4 mb-6">
        <h2 class="text-xl font-bold mb-2">Referral Information</h2>
        <div class="space-y-3 border-y text-gray-700">
            <p><strong>Hospital Name:</strong> {{ $referral->hospital_name }}</p>
            <p><strong>Doctor Name:</strong> {{ $referral->doctor_name }}</p>
            <p><strong>Diagnosis:</strong> {{ $referral->diagnosis }}</p>
            <p><strong>Date Referred:</strong> {{ Carbon\Carbon::parse($referral->created_at)->format('F d, Y') }}</p>
        </div>
    </div>
